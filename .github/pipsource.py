...
def adjust_version():
    ...
    '''
    pip = check_output(['curl', '-sL', 'https://test.pypi.org/simple/clingox']).decode()
    version = '1.0.0'

    assert version is not None

    post = 0
    for m in finditer(r'clingox-{}\.tar\.gz'.format(escape(version)), pip):
        post = max(post, 1)

    for m in finditer(r'clingox-{}\.post([0-9]+)\.tar\.gz'.format(escape(version)), pip):
        post = max(post, int(m.group(1)) + 1)

    for m in finditer(r'clingox-{}\.post([0-9]+).*\.whl'.format(escape(version)), pip):
